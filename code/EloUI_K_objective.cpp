...
// NumericVector objective_core_cpp2(const double par_K, IntegerVector y,
//                          IntegerVector student,IntegerVector item,
//                          IntegerVector skill_i,IntegerVector skill_n,
//                          IntegerVector skill, const int nG,
//                          const int nI, const int nK, std::string toreturn) { 
// [[Rcpp::export]]


  
SEXP objective_core_cpp(NumericVector par_K, List d, const int nG, const int nI, const int nK, std::string toreturn){ 
  IntegerVector y = d["y"];
  ...
  // std::cout << "t\ty[t]\tstudent_t\titem_t\tobj_t\tgrad_t\tpcorr\terror\tval_student_t\tval_item_t\n";
  ...
  for(int t=0; t<N; t++) {
    ...
    objval_t = -( y[t] * log(pcorr) + (1-y[t]) * log (1-pcorr) );
    ...
    sig_ = 1;
    values_stu[student_t] += sig_ * K_ * error;// #(y[t] - pcorr);
    counts_stu[student_t] ++;
    if(toreturn=="gradient") {
      if(n_>0) {
        grad[0] -= error * grad_errors_stu[student_t];
        grad_t -= error * grad_errors_stu[student_t];
      }
    ...
    sig_ = -1;
    values_itm[item_t] += sig_ * K_ * error;
    counts_itm[item_t] ++;
    if(toreturn=="gradient") {
      if(n_>0) {
        grad[0] -= error * grad_errors_itm[item_t];
        grad_t -= error * grad_errors_itm[item_t];
      }
      grad_errors_itm[item_t] += error;
    }
    // std::cout << t <<"\t"<< y[t]<<"\t"<<student_t<<"\t"<<item_t<<"\t"<<objval_t<<"\t"<<grad_t<<"\t"<<pcorr<<"\t"<<error<<"\t"<<values_stu[student_t]<<"\t"<<values_itm[item_t]<<"\n";
  }
